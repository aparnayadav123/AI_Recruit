...
const ShortlistReport: React.FC<ShortlistReportProps> = ({ searchQuery = '' }) => {
      ...
      try {
        ...
        candidates.forEach((c: any) => {
          const formatted: ShortlistedCandidate = {
            ...
            currentCompany: c.currentOrganization || 'Not specified',
            noticePeriod: (c.noticePeriod !== null && c.noticePeriod !== undefined && c.noticePeriod !== '')
              ? `${c.noticePeriod} days` : 'Not specified',
        ...
        // Rank each group by fit score, highest first. Tie-break on name so the
        // ordering is stable across re-fetches.
        Object.keys(groupedByRole).forEach(key => {
          groupedByRole[key].sort((a, b) => {
  ...
  // Non-destructive: remove the candidate from THIS shortlist view only.
  // (Previously this permanently DELETEd the candidate record — a dangerous mismatch
  // with the "Remove from shortlist" label.)
  const handleRemove = (role: string, candidateId: string) => {
    if (confirm('Remove this candidate from the shortlist view? (This does NOT delete the candidate.)')) {
  ...
  const handleExportPDF = () => {
    ...
    if (printWindow) {
      const content = document.querySelector('.space-y-4')?.innerHTML || '';
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 bg-white p-3 rounded-xl border border-slate-300 shadow-sm transition-all">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-600 font-bold mt-1 uppercase tracking-wider leading-none">Generated on {new Date().toLocaleDateString()}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={handlePrint} className="p-2 bg-slate-50 text-slate-600 rounded-lg hover:bg-slate-100 transition border border-slate-300">
            <Printer size={14} />
          </button>
        ...
        {[
          ...
          >
            <div className="flex items-center gap-3 relative z-10">
              ...
              <div>
                <p className={`text-[10px] font-black uppercase tracking-widest leading-none mb-1 ${selectedFilter === stat.id ? 'text-white/70' : 'text-gray-600'}`}>{stat.label}</p>
      ...
      {Object.entries(filteredData).map(([role, candidates]) => (
        <div key={role} className="bg-white rounded-xl border border-slate-300 shadow-sm overflow-hidden mb-4">
          {/* Section header */}
          <div className="px-4 py-2.5 bg-slate-50 border-b border-slate-300 flex justify-between items-center">
            <h3 className="text-[10px] font-black text-gray-900 uppercase tracking-widest flex items-center gap-2">
              <div className="w-1 h-3 bg-blue-600 rounded-full"></div>
              {role}
            </h3>
            <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-white border border-blue-100 text-blue-600 uppercase tracking-widest">
              {candidates.length} {candidates.length === 1 ? 'Profile' : 'Profiles'}
            </span>
          </div>

          {/* Column headers — keep rows aligned vertically */}
          <div className="hidden md:grid grid-cols-12 gap-3 px-4 py-2 bg-slate-50/60 border-b border-slate-300 text-[9px] font-black text-slate-600 uppercase tracking-widest">
          ...
          <div className="divide-y divide-slate-300">
            {candidates.map((candidate) => (
              <div
                key={candidate.id}
                className="px-4 py-3 hover:bg-slate-50/60 transition-colors grid grid-cols-12 gap-3 items-center"
              >
                {/* Candidate (4) — avatar + name + company */}
                <div className="col-span-12 md:col-span-4 flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-blue-500 font-bold truncate leading-tight mt-0.5">
                      {highlightText(candidate.currentCompany)}
                    </p>
                  </div>
                </div>
                ...
                <div className="col-span-6 md:col-span-2 min-w-0">
                  <div className="md:hidden text-[9px] font-black text-slate-600 uppercase tracking-widest mb-0.5">Experience</div>
                  <p className="text-[11px] font-bold text-gray-900 truncate">{candidate.experience}</p>
                </div>

                {/* Notice (2) */}
                <div className="col-span-6 md:col-span-2 min-w-0">
                  <div className="md:hidden text-[9px] font-black text-slate-600 uppercase tracking-widest mb-0.5">Notice</div>
                  <p className="text-[11px] font-bold text-gray-900 truncate">{candidate.noticePeriod}</p>
                </div>

                {/* Notes (2) — muted, single line */}
                <div className="col-span-12 md:col-span-2 min-w-0">
                  {candidate.notes && candidate.notes !== 'No notes available.' ? (
                    <p
                ...
                {/* Actions (2) — right-aligned icon trio with breathing room */}
                <div className="col-span-12 md:col-span-2 flex items-center justify-end gap-1 shrink-0 pr-1">
                  <button
                  ...
                  {candidate.interviewStatus === 'Scheduled' ? (
                    <button
                      ...
                      title="Interview scheduled — edit"
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-white border-b border-slate-300 px-6 py-4 flex items-center justify-between">
                ...
                <button onClick={() => setShowAssessmentModal({ candidate: null, matrix: null })} className="text-gray-600 hover:text-gray-600">
                  <X className="w-6 h-6" />
                </button>
              </div>
              <div className="p-6">
                <div className="mb-6">
                  ...
                  <p className="text-gray-600">{showAssessmentModal.candidate.role}</p>
                </div>
                <div className="space-y-4">
                  {showAssessmentModal.matrix && showAssessmentModal.matrix.skillMetrics ? (
                    ...
                    <p className="text-center py-8 text-gray-600 italic">No detailed skill metrics available for this candidate.</p>
                  )}
                  ...
                  <div className="mt-6 p-4 bg-indigo-50 rounded-lg flex justify-between items-center">
                    ...
                    {showAssessmentModal.matrix && (
                      <div className="text-right">
                        <p className="text-xs text-gray-600 uppercase font-bold mb-1">Recommended Role</p>
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    <ExternalLink className="absolute left-3 top-2.5 w-5 h-5 text-gray-600" />
                    <input
                      type="text"
