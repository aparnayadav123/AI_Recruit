...
interface Interview {
    ...
    notes: string;
    status: string;
    meetingLink?: string; // Zoom / Meet / location entered when scheduling
}
        ...
        <div className="fixed bottom-6 right-6 z-50 animate-in slide-in-from-bottom-5 fade-in duration-300">
            <div className="bg-white border-l-4 border-indigo-600 border-2 border-blue-200 rounded-lg shadow-2xl p-4 w-96 flex flex-col gap-3 relative">
                ...
                {upcomingInterview.meetingLink && (
                    <div className="flex items-center gap-2 text-[12px] text-indigo-700 break-all bg-indigo-50 border border-indigo-100 rounded-lg px-3 py-2">
                        <Video className="w-3.5 h-3.5 shrink-0" />
                        <span className="truncate">{upcomingInterview.meetingLink}</span>
                    </div>
                )}
                ...
                <div className="flex gap-2 mt-1">
                    <button
                        onClick={() => {
                            ...
                            if (upcomingInterview.meetingLink) {
                                // Open the actual meeting (Zoom / Meet / etc.) in a new tab.
                                window.open(upcomingInterview.meetingLink, '_blank', 'noopener,noreferrer');
                            } else {
                    ...
                    >
                        {upcomingInterview.meetingLink ? 'Join Meeting' : 'View Details'}
                    </button>
                    <button
                        onClick={() => setUpcomingInterview(null)}
