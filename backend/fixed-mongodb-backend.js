...
async function connectToMongoDB() {
  try {
    ...
    // Drop and recreate collections without strict validation
    try {
      await db.collection('jobs').drop();
      console.log('🗑️  Dropped existing jobs collection');
    } catch (error) {
      console.log('📋 Jobs collection does not exist yet');
    }
    
    await db.createCollection('jobs');
    console.log('✅ Created jobs collection without validation');
    
    try {
      await db.collection('candidates').drop();
      console.log('🗑️  Dropped existing candidates collection');
    } catch (error) {
      console.log('📋 Candidates collection does not exist yet');
    }
    
    await db.createCollection('candidates');
    console.log('✅ Created candidates collection without validation');
    
    try {
      await db.collection('job_applications').drop();
      console.log('🗑️  Dropped existing job_applications collection');
    } catch (error) {
      console.log('📋 Job_applications collection does not exist yet');
    }
    
    await db.createCollection('job_applications');
    console.log('✅ Created job_applications collection without validation');
    
    console.log('🗄️  Database: recruitai');
    console.log('📊 Collections: candidates, jobs, job_applications (no validation)');
...
app.get('/api', (req, res) => {
  res.json({
    message: 'RecruitAI Backend API (Fixed MongoDB)',
    version: '1.0.0',
    database: 'MongoDB',
    connection: mongoUri,
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.json({
      ok: true,
      service: 'recruitai-agent-mongodb-fixed',
      database: 'MongoDB',
      connected: true,
      timestamp: new Date().toISOString()
    });
...
app.listen(port, async () => {
  ...
  console.log(`🚀 RecruitAI Fixed MongoDB Backend running on http://localhost:${port}`);
  ...
  console.log(`🗄️  Database: MongoDB (No Validation - Flexible)`);
