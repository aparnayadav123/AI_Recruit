...
public class ZoomService {
    ...
    public String createMeeting(String topic, String startTime, int duration) {
        ...
        // Zoom not configured (no credentials) → still return a usable, no-signup video link
        // (Jitsi Meet) so interviews always have a join link without external API setup.
        if (token == null)
            return fallbackMeetingLink(topic);
        ...
        // Any Zoom failure → fall back to a working link rather than leaving none.
        return fallbackMeetingLink(topic);
    }

    /**
     * Generates a free, no-signup Jitsi Meet room link. Used when Zoom isn't configured
     * (or fails) so an interview always has a joinable video link.
     */
    private String fallbackMeetingLink(String topic) {
        String slug = (topic == null ? "Interview" : topic).replaceAll("[^A-Za-z0-9]+", "");
        if (slug.isBlank()) {
            slug = "Interview";
        }
        String suffix = Long.toString(Math.abs(java.util.UUID.randomUUID().getMostSignificantBits()), 36);
        return "https://meet.jit.si/RecruitAI-" + slug + "-" + suffix;
    }
}
