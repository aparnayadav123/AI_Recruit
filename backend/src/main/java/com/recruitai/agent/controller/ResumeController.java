...
public class ResumeController {
    ...
    /**
     * Attach a resume to an existing candidate (from their details page) and
     * re-score them from it — without creating a new candidate by email.
     */
    @PostMapping("/upload-for/{candidateId}")
    public ResponseEntity<?> uploadResumeForCandidate(@PathVariable String candidateId,
            @RequestParam("file") MultipartFile file,
            @RequestParam(value = "source", defaultValue = "UPLOAD") String source,
            @RequestParam(value = "assignedBy", required = false) String assignedBy) {
        try {
            Candidate candidate = resumeService.attachResumeToCandidate(file, candidateId, source, assignedBy);
            return ResponseEntity.ok(candidate);
        } catch (IOException e) {
            e.printStackTrace();
            return ResponseEntity.status(500).body("Internal Server Error: " + e.getMessage());
    ...
    @PostMapping("/backfill-linkedin-urls")
    public ResponseEntity<?> backfillLinkedinUrls() {
        return ResponseEntity.ok(resumeService.backfillLinkedinUrls());
    }
}
