...
public class JobController {
    ...
    @Autowired
    private com.recruitai.agent.service.RediscoveryService rediscoveryService;
    ...
    public ResponseEntity<List<Job>> getPublicJobs() {
        // Only return Open jobs that have been explicitly approved for the live careers page
        List<Job> publishedJobs = jobService.getPublishedOpenJobs();
        return ResponseEntity.ok(publishedJobs);
    }

    @PostMapping("/{id}/publish-to-careers")
    public ResponseEntity<Job> publishToCareers(@PathVariable String id) {
        Job published = jobService.publishJob(id);
        return ResponseEntity.ok(published);
    }

    @PostMapping("/{id}/unpublish-from-careers")
    public ResponseEntity<Job> unpublishFromCareers(@PathVariable String id) {
        Job unpublished = jobService.unpublishJob(id);
        return ResponseEntity.ok(unpublished);
    }

    // ---------------- Rediscovery (Module 8/9) ----------------
    @GetMapping("/{id}/suggested-candidates")
    public ResponseEntity<?> getSuggestedCandidates(@PathVariable String id) {
        return ResponseEntity.ok(rediscoveryService.getSuggestedForJob(id));
    }

    // ---------------- Lifecycle (Phase 2) ----------------
    @PostMapping("/{id}/close")
    public ResponseEntity<Job> closeJob(@PathVariable String id) {
        return ResponseEntity.ok(jobService.closeJob(id));
    }

    @PostMapping("/{id}/reopen")
    public ResponseEntity<Job> reopenJob(@PathVariable String id) {
        return ResponseEntity.ok(jobService.reopenJob(id));
    }

    @PostMapping("/{id}/archive")
    public ResponseEntity<Job> archiveJob(@PathVariable String id) {
