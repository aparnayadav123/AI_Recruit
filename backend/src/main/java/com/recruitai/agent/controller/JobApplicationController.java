...
public class JobApplicationController {
    ...
    // ---------------- Rejection management (Phase 2) ----------------

    @GetMapping("/rejected")
    public ResponseEntity<List<JobApplication>> getRejectedApplications() {
        return ResponseEntity.ok(
                applicationService.getApplicationsByStatus(JobApplication.ApplicationStatus.REJECTED));
    }
    ...
    @PostMapping("/{id}/stages/init")
    public ResponseEntity<JobApplication> initStages(@PathVariable String id) {
        return ResponseEntity.ok(applicationService.initStages(id));
    }

    @PostMapping("/{id}/stages/record")
    public ResponseEntity<JobApplication> recordStage(
            @PathVariable String id,
            @RequestBody java.util.Map<String, Object> body) {
        String stageName = (String) body.get("stage");
        String outcome = (String) body.get("outcome");
        String interviewer = (String) body.get("interviewer");
        Integer rating = body.get("rating") == null ? null : ((Number) body.get("rating")).intValue();
        String feedback = (String) body.get("feedback");
        String notes = (String) body.get("notes");
        return ResponseEntity.ok(
                applicationService.recordStage(id, stageName, outcome, interviewer, rating, feedback, notes));
    }
