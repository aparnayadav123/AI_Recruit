...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function directExtractLinkedInDOM() {
        ...
        // 3. Top Card Headline from DOM (Targeted selectors with candidate name rejection)
        const headlineCandidateSelectors = [
        ...
        for (const sel of headlineCandidateSelectors) {
            const headEls = document.querySelectorAll(sel);
            for (const headEl of headEls) {
                if (headEl.closest('h1')) continue;
                const txt = (headEl.innerText || '').trim();
        ...
        let primaryRoleCandidate = extractCleanRole(data.rawHeadline || '', data.about, [], data.name);
        if (isInvalidHeadline(primaryRoleCandidate, data.name)) primaryRoleCandidate = '';
        if (expRoleTitle && (!primaryRoleCandidate || primaryRoleCandidate === 'Software Engineer' || primaryRoleCandidate === 'Software Developer')) {
            const cleanExp = extractCleanRole(expRoleTitle, '', [], data.name);
            if (cleanExp && cleanExp.length > 2 && !isInvalidHeadline(cleanExp, data.name) && cleanExp !== 'Software Engineer') {
                primaryRoleCandidate = cleanExp;
            } else if (expRoleTitle.length > 2 && !/full-time|part-time|contract/i.test(expRoleTitle) && !isInvalidHeadline(expRoleTitle, data.name)) {
                primaryRoleCandidate = expRoleTitle;
            }
        }
        if (!primaryRoleCandidate && jsonLdRole && !isInvalidHeadline(jsonLdRole, data.name)) {
            primaryRoleCandidate = jsonLdRole;
        }

        data.primaryRole = primaryRoleCandidate || 'Business Development Executive';
